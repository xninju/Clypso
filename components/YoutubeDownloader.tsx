...
export default function YoutubeDownloader({ onUrlChange, initialUrl, externalUrl }: { onUrlChange?: (url: string) => void; initialUrl?: string; externalUrl?: string }) {
  ...
  const handleFetch = async (urlOverride?: string) => {
    ...
    setInfo(null);
    try {
      const res = await axios.post(`/api/youtube/info`, { url: target }, { timeout: 45000 });
      setInfo(res.data);
    } catch (e: unknown) {
  ...
  // Auto-fetch when mounted with externalUrl
  useEffect(() => {
  ...
  return (
    <div>
      ...
      {controlled && loading && (
        <div className="flex items-center gap-2 text-sm text-[#717171] py-2">
          <div className="w-4 h-4 border-2 border-[#555] border-t-[#f1f1f1] rounded-full animate-spin" />
          Fetching...
        </div>
      )}
